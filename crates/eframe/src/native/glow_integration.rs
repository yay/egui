...
struct GlutinWindowContext {
    ...
    /// Owns the WGL config's bootstrap HDC. Keep last so configs, contexts, and surfaces drop
    /// first. This hidden resource window is never an egui viewport or a presentation target.
    _bootstrap_window: Option<Window>,
}
    ...
    fn run_ui_and_paint(
...

        {
            ...
            if is_immediate && viewport_id != ViewportId::ROOT {
                ...
                if logical_root && viewport.ids.parent == ViewportId::ROOT {
                    return Ok(EventResult::RepaintLogicalRootNow);
                }
...
impl GlutinWindowContext {
    ...
    unsafe fn new(
        ...
        */
        ...
        let (mut window, gl_config) = if logical_root {
            windowless::create_display(egui_ctx, event_loop, config_template_builder)?
        } else {
            let (window, config) = glutin_winit::DisplayBuilder::new()
                ...
                .map_err(|err| {
                    crate::Error::NoGlutinConfigs(config_template_builder.build(), err)
                })?;
        ...
        // The private WGL bootstrap stays alive because glutin also queries its HDC later.
        ...
        viewports.insert(
            ...
            Viewport {
                ...
                requested_active: !logical_root && root_builder.active.unwrap_or(true),
    ...
    /// crash on Windows. A windowless app restores its controller target without querying window
    /// surfaces. A windowed app uses a surviving viewport (preferably the root), or detaches if
    /// none survive. On failure, returns an error so callers keep the surfaces alive.
    fn make_removed_viewports_not_current(
        &mut self,
        viewport_output: &OrderedViewportIdMap<ViewportOutput>,
    ) -> Result {
        if self.logical_root {
            // CGL's surface query panics when the context is surfaceless. Restore the controller
            // before querying any window surface, and never drop a current surface on failure.
            return self.make_controller_current();
        }
        ...
        } else if let Some(current) = current_gl_context.as_ref() {
            current.make_not_current_in_place()?;
        }
        Ok(())
    }

    /// Drops undeclared viewports after moving the GL context away from their surfaces.
    /// A context-switch failure leaves the viewport maps intact and is returned to the event loop.
