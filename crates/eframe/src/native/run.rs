...
impl<T: WinitApp> ApplicationHandler<UserEvent> for WinitAppWrapper<T> {
    ...
    fn new_events(&mut self, event_loop: &ActiveEventLoop, cause: winit::event::StartCause) {
        ...
        // Timer dispatch can run app code, including creation of immediate child windows.
...
/// Paces immediate controller requests, including requests forwarded by immediate children.
/// Explicit delayed requests keep their own deadlines and should not call this helper.
fn logical_root_deadline(requested: Instant, last: Option<Instant>, interval: Duration) -> Instant {
    last.map_or(requested, |last| requested.max(last + interval))
}

#[cfg(test)]
mod windowless_tests {
    use super::*;

    #[test]
    fn immediate_requests_are_paced_without_delaying_later_deadlines() {
        let now = Instant::now();
        let interval = Duration::from_millis(16);
        assert_eq!(logical_root_deadline(now, None, interval), now);
        assert_eq!(
            logical_root_deadline(now, Some(now), interval),
            now + interval
        );
        let later = now + Duration::from_secs(1);
        assert_eq!(logical_root_deadline(later, Some(now), interval), later);
    }
}
